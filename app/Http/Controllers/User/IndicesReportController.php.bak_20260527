...
class IndicesReportController extends Controller
{
        ...
        if (!empty($request->input('indices'))) {
            ...
            foreach ($indices as $index) {
                $closingValues = IndicesDetail::where('correlation_new', $index)
                    ...
                    ->toArray();
                $index_name = IndicesMaster::where('corelation', $index)->where('status',1)->value('name');
                ...
                if (!empty($closingValues)) {
                    ...
                    $data['indices_vals'][$index_name] = $chartData;
                } else {
    ...
    public function boomers(Request $request)
    {
        ...
        if (isset($getdata) && count($getdata) > 0) {
            ...
            $data['indices_name'] = $indices = $getdata['indices'];
            $startDate = $this->getlastdate($month, $year);
            $endDate = $this->getlastdate($month_second, $year_second);

            $data['indices_records'] = DB::table('indices_master')->whereIn('corelation', $indices)->get();
        }
        ...
        if ($request->has('indices')) {
            ...
            $request->validate(
                ...
                [
                    ...
                    'limit' . 'required' => 'Please enter enter limit for Searching',
            ...
            //     $results_classification_scrips = DB::select("
            //     SELECT 
            //         mpx_fc1.entry_date, 
            //         mpx_fc2.entry_date, 
            //         mpx_fc1.scrip_name, 
            //         mpx_fc1.correlation_new, 
            //         SUM(mpx_fc1.percentage) as percentage_new, 
            //         SUM(mpx_fc2.percentage) as percentage_old 
            //     FROM 
            //         mpx_indices_composition as mpx_fc1 
            //     INNER JOIN 
            //         mpx_indices_composition as mpx_fc2 
            //     ON 
            //         mpx_fc1.scrip_name = mpx_fc2.scrip_name 
            //     WHERE 
            //         mpx_fc1.correlation_new IN ('$indicesString') 
            //         AND mpx_fc2.correlation_new IN ('$indicesString') 
            //         AND mpx_fc1.entry_date = '$endDate' 
            //         AND mpx_fc2.entry_date = '$startDate' 
            //         AND mpx_fc1.publish = 'y' 
            //         AND mpx_fc2.publish = 'y' 
            //     GROUP BY 
            //         mpx_fc1.scrip_name
            // ");

            $results_classification_scrips = DB::select("SELECT 
scrip_name,correlation_new,
SUM(CASE WHEN entry_date = '$endDate' THEN percentage ELSE 0 END) as percentage_new,
SUM(CASE WHEN entry_date = '$startDate' THEN percentage ELSE 0 END) as percentage_old
FROM 
mpx_indices_composition
WHERE 
correlation_new IN ('$indicesString') 
AND entry_date IN ('$endDate', '$startDate')
GROUP BY 
scrip_name");

            // dd($indicesString."  ".$startDate." ".$endDate);

            // dd($results_classification_scrips);

            /*--------for industry--------*/

            //     $results_classification_industry = DB::select("
            //     SELECT 
            //         mpx_fc1.entry_date, 
            //         mpx_fc2.entry_date, 
            //         mpx_fc1.industry, 
            ...
            $results_classification_industry = DB::select("SELECT 
industry,correlation_new,
SUM(CASE WHEN entry_date = '$endDate' THEN percentage ELSE 0 END) as percentage_new,
SUM(CASE WHEN entry_date = '$startDate' THEN percentage ELSE 0 END) as percentage_old
FROM 
mpx_indices_composition
...
industry");
    ...
    public function index_vs_NAV(Request $request)
    {
        ...
        $data['indices'] = IndicesMaster::where('status', 1)->get();
        $data['indices_vals'] = [];
        $data['schemes'] = FundMaster::where('status', 1)->get();
        // dd($data);
        // dd($request->all());
        $disclaimerQuery = DB::table('fund_watch_disclaimer')->where('status', 1)->first();
