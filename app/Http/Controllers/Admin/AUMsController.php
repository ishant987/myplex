...
class AUMsController extends BaseController
{
    public $className;
    ...
    public function index(Request $request)
    {
        ...
        $sortbyArr = ['ce_id' => __('admin.insertion_txt'), 'fund_code' => __('admin.fund.code_txt'), 'corpus_entry' => __('admin.value_txt'), 'percentage_change' => __('admin.percentage_change_txt'), 'corpus_change' => __('admin.fund.corpus_change_txt')];
        $orderbyArr = ['asc' => 'ASC', 'desc' => 'DESC'];

        $showEntryArr = ['value' => __('admin.sw_entry.options.value'), 'text' => __('admin.sw_entry.options.text')];

        $roleRights = ['delete' => App::hasAccessToMethod($this->className, 'admin.aums.delete')];

        return view('themes.backend.pages.fund.aum-list', compact('dataListModel', 'data', 'listDataAtrArr', 'moduleAtrArr', 'sortbyArr', 'orderbyArr', 'fltrDataArr', 'perPage', 'sortBy', 'orderBy', 'showEntryArr', 'lastSavedDate', 'roleRights'));
    }
    ...
    public function deletedata()
    {
        ...
        try {
            DB::beginTransaction();
            $lastSavedDate = SettingsModel::getSettingValue('aum');
            $delModel = CorpusEntry::where('entry_date', $lastSavedDate)->delete();
            if ($delModel > 0) {
                $fdMdl = CorpusEntry::orderBy('entry_date', 'desc')->first();
                if($fdMdl){
