...
class MgrFundClassificationMPX extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'migration:fundclassificationmyplexus';
     ...
     * @var string
     */
    protected $description = 'Take data from Tb_Portfolio_Theory table.';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $totRecords = $mgrDb->table('Tb_Portfolio_Theory')->select('fund_id')->count();
            if ($totRecords > 0) {
                ...
                for ($i = 1; $i <= $noOfLot; $i++) {
                    ...
                    $dataRows = $mgrDb->table('Tb_Portfolio_Theory')->offset($offset)->take($this->fetchRecords)->get();
                    // print_r($dataRows);die();
                    foreach ($dataRows as $record) {
                        // $this->info("title : " . $record->title);
                        $store = new FundClassification;
                        if (!$store->where('title', $record->p_theory_title)->exists()) {
                            /*Insert code here*/
                            $store->title = $record->p_theory_title;
                            $store->description = $record->p_theory_des;
                            $store->file = $record->upload_file;
                            $store->created_id = $defSprAdmnId;
                            $store->updated_id = $defSprAdmnId;
                            $store->created_at = date($commonconstants['y_m_d_frmt'], strtotime($record->p_theory_date)) . ' 11:00:00';
                            ...
                            // $this->info("Data exists - title : " . $record->p_theory_title);
                        }
                        $bar->advance();
                    }
                }
