...
class MgrFundTaxationMPX extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'migration:fundtaxationmyplexus';
     ...
     * @var string
     */
    protected $description = 'Take data from Tb_M_Fund_Watch table.';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $totRecords = $mgrDb->table('Tb_M_Fund_Watch')->select('fund_id')->count();
            if ($totRecords > 0) {
                ...
                for ($i = 1; $i <= $noOfLot; $i++) {
                    ...
                    $dataRows = $mgrDb->table('Tb_M_Fund_Watch')->offset($offset)->take($this->fetchRecords)->get();
                    // print_r($dataRows);die();
                    foreach ($dataRows as $record) {
                        // $this->info("title : " . $record->title);
                        $store = new FundTaxation;
                        if (!$store->where('title', $record->taxation_title)->exists()) {
                            /*Insert code here*/
                            $store->title = $record->taxation_title;
                            $store->description = $record->taxation_des;
                            $store->file = $record->upload_file;
                            $store->created_id = $defSprAdmnId;
                            $store->updated_id = $defSprAdmnId;
                            $store->created_at = date($commonconstants['y_m_d_frmt'], strtotime($record->taxation_date)) . ' 11:00:00';
                            ...
                            // $this->info("Data exists - title : " . $record->taxation_title);
                        }
                        $bar->advance();
                    }
                }
