...
class MgrFundCompValueMonthlySF extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'migration:fundcompvaluemonthlysitefolks';
     ...
     * @var string
     */
    protected $description = 'Take data from tb_fundcomposition table.';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $totRecords = $mgrDb->table('tb_fundcomposition')->select('EntryDate')->count();
            if ($totRecords > 0) {
                ...
                for ($i = 1; $i <= $noOfLot; $i++) {
                    ...
                    $dataRows = $mgrDb->table('tb_fundcomposition')->offset($offset)->take($this->fetchRecords)->get();
                    // print_r($dataRows);die();
                    foreach ($dataRows as $record) {
                        // $this->info("scrip_name : " . $record->ScripName);
                        $store = new FundComposition;
                        if (!$store->where(['scrip_name' => $record->ScripName, 'entry_date' => $record->EntryDate, 'fund_code' => $record->FundCode])->exists()) {
                            /*Insert code here*/
                            ...
                            $store->content_per = $record->ContentPer;
                            $store->amount = $record->Amount;
                            $store->no_of_shares = $record->NoOfShares;
                            $store->indices_per = $record->IndicesPer;
                            $store->publish = $yVal;
