...
class MgrAUMsValueMonthlyMPX extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'migration:aumsvaluemonthlymyplexus';
     ...
     * @var string
     */
    protected $description = 'Take data from TbCorpusEntry table.';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $totRecords = $mgrDb->table('TbCorpusEntry')->select('EntryDate')->count();
            if ($totRecords > 0) {
                ...
                $lastCurrencyMigrateOn = SettingsModel::getSettingValue('aum_migrate');
                for ($i = 1; $i <= $noOfLot; $i++) {
                    $offset = ($i - 1) * $this->fetchRecords;
                    $dataRows = $mgrDb->table('TbCorpusEntry')->whereDate('EntryDate', '>=', $lastCurrencyMigrateOn)->offset($offset)->take($this->fetchRecords)->orderBy('EntryDate', 'ASC')->get();
                    // print_r($dataRows);die();
                    foreach ($dataRows as $record) {
                        // $this->info("fund_code : " . $record->FundCode);
                        $storeEntryDate = date($commonconstants['y_m_d_frmt'], strtotime($record->EntryDate));
                        if ($lastCurrencyMigrateOn != $storeEntryDate) {
                            SettingsModel::where('option_key', 'aum_migrate')->update(['option_value' => $storeEntryDate, 'updated_id' => $defSprAdmnId]);
                        } else {
                            $lastCurrencyMigrateOn = SettingsModel::getSettingValue('aum_migrate');
                        }
                        $store = new CorpusEntry;
                        if (!$store->where(['fund_code' => $record->FundCode, 'entry_date' => $record->EntryDate])->exists()) {
                            ...
                            if ($store->save()) {
                                ...
                                // $this->info("Data Not inserted - fund_code : " . $record->FundCode);
                            }
                        } else {
                            // $this->info("Data exists - fund_code : " . $record->FundCode);
                        }
                        $bar->advance();
                    }
                }
