<?php

namespace App\Console\Commands;

use Illuminate\Console\Command;
use Illuminate\Database\QueryException;

use Log;
use DB;
use Carbon\Carbon;

use App\Models\FundMaster;

class MgrFundMasterSF extends Command
{
    /**
     * The name and signature of the console command.
     *
     * @var string
     */
    protected $signature = 'migration:fundmastersitefolks';
    
    public $fetchRecords = 100;

    /**
     * The console command description.
     *
     * @var string
     */
    protected $description = 'Take data from tb_fundmaster table.';

    /**
     * Create a new command instance.
     *
     * @return void
     */
    public function __construct()
    {
        parent::__construct();
    }

    /**
     * Execute the console command.
     *
     * @return mixed
     */
    public function handle()
    {
        Log::channel('cronlog')->info(__CLASS__ . ' - START ');
        $this->line('');
        $this->line(__('message.command_start_txt'));
        $this->line('');

        try {
            $mgrDb = DB::connection('mysqlisf');

            $totRecords = $mgrDb->table('tb_fundmaster')->select('FundID')->count();
            if ($totRecords > 0) {
                $commonconstants = Config('commonconstants');
                $date = new Carbon;
                $dateFormatted = $date->format($commonconstants['db_dt_tm_frmt']);
                $defSprAdmnId = $commonconstants['def_super_admin_id'];

                $this->info("Total : " . $totRecords);
                $bar = $this->output->createProgressBar($totRecords);
                $bar->start();

                $noOfLot = 1;
                if ($totRecords > $this->fetchRecords) {
                    $noOfLot = ceil($totRecords / $this->fetchRecords);
                }else{
                    $this->fetchRecords = $totRecords;
                }
                // echo $noOfLot;die();
                for ($i = 1; $i <= $noOfLot; $i++) {
                    $offset = ($i - 1) * $this->fetchRecords;
                    $dataRows = $mgrDb->table('tb_fundmaster')->offset($offset)->take($this->fetchRecords)->get();
                    // print_r($dataRows);die();
                    foreach ($dataRows as $record) {
                        // $this->info("FundID : " . $record->FundID);
                        $store = new FundMaster;
                        if (!$store->where('fund_id', $record->FundID)->exists()) {
                            /*Insert code here*/
                            $store->fund_id = $record->FundID;
                            $store->fund_name = $record->NameOfFund;
                            $store->fund_code = $record->FundCode;
                            $store->fund_manager = $record->FundManager;
                            $store->fund_type_id = $record->FundTypeID;
                            $store->fund_term_id = $record->FundTermID;
                            $store->face_value = $record->FaceValue;
                            $store->risk_free_return = $record->RiskFreeReturn;
                            $store->fund_opened = $record->OpDate;
                            $store->period = $record->Period;
                            $store->remarks = $record->Remarks;
                            $store->cost = $record->Cost;
                            $store->indices_name = $record->IndicesName;
                            $store->fund_house = $record->FundHouse;
                            $store->classification = $record->Classification;
                            $store->created_id = $defSprAdmnId;
                            $store->updated_id = $defSprAdmnId;
                            $store->created_at = $dateFormatted;
                            $store->updated_at = $dateFormatted;
                            $store->migration_at = $dateFormatted;
                            // print_r($store);die();
                            if ($store->save()) {
                                // $this->info("Inserted ID : " . $store->fund_id);
                            } else {
                                // $this->info("Data Not inserted - FundID : " . $record->FundID);
                            }
                        }
                        else{
                            // $this->info("Data exists - FundID : " . $record->FundID);
                        }
                        $bar->advance();
                    }
                }
                $bar->finish();
            } else {
                $this->error(__('message.data_not_available'));
            }
        } catch (QueryException $exception) {
            $this->error($exception);
            // $this->error(__('message.error.something_wrong'));
        }

        $this->line('');
        $this->line(__('message.command_end_txt'));
        $this->line('');
        Log::channel('cronlog')->info(__CLASS__ . ' - END ');
    }
}
