                ...
                <div class="tab-content">
                    <div id="daily-price" class="tab-pane fade in" :class="{'active show':currentTab == 'daily_price'}">
                        <div class="row investing-tools">
                            ...
                            <!-- <div class="col-md-3 col-sm-12 invst-wrap select2-styles" v-if="enablePriceCustomDates">
                                <div class="invst-fields mt-0">
                                <label class="w-100">To Date</label>
                                <Datepicker v-model="selectedToDate" :format="'dd/MM/yyyy'" :enableTimePicker="false" :autoApply="true"></Datepicker>
                                </div>
                            </div> -->
  ...
  methods: {
   ...
   toggleDataSeries(e) {
    ...
    priceCompare(){
        ...
        if(!this.enablePriceCustomDates){
            ...
            data.from_date = moment(this.selectedDateRange[0]).format('YYYY-MM-DD')
            data.to_date = moment(this.selectedDateRange[1]).format('YYYY-MM-DD')
        }
    ...
    ratioCompare(){
        ...
        if(!this.enableRatioCustomDates){
            data.from_date = moment().subtract(1, 'days').subtract(this.selectedRatioDuration, 'months').format('YYYY-MM-DD')
            data.to_date = moment().subtract(1, 'days').format('YYYY-MM-DD')
        }else if(this.enableRatioCustomDates){
            data.from_date = moment(this.selectedRatioDateRange[0]).format('YYYY-MM-DD')
            data.to_date = moment(this.selectedRatioDateRange[1]).format('YYYY-MM-DD')
        }
        ...
        axios.get('/api/v1/compare-ratios', { params: data })
            .then(response => {
                ...
                that.chart_ratio.options.data[0].dataPoints.push({ y:parseFloat(yVal) ,label:title1});
                }
                if(Object.keys(graph_data[1]).length){
                let yVal = graph_data[1][data.compare_type].toFixed(2)                
                that.chart_ratio.options.data[0].dataPoints.push({ y: parseFloat(yVal),label:title2});
                }
  ...
  computed: {
    ...
    disableCompareRatio(){
        if (this.selectedFund1Ratio == '' || this.selectedFund2Ratio == '') {
            ...
            return true
        }else if(this.enableRatioCustomDates && !this.selectedRatioDateRange){
            return true
        }
  ...
  mounted() {
    ...
    myPromise.then( async (resolve, reject) => {
        if(compare_price_type && val1 && val2){
            ...
            if((this.selectedComparePriceType == 'scheme_scheme' || this.selectedComparePriceType == 'scheme_index' || this.selectedComparePriceType == 'scheme_currency')){
                this.selectedScheme1 = that.funds.filter(function(el) { return el.fund_code == decodeURIComponent(val1) })[0]
            }
            if((this.selectedComparePriceType == 'scheme_scheme')){
                this.selectedScheme2 = that.funds.filter(function(el) { return el.fund_code == decodeURIComponent(val2) })[0]
            }
            if((this.selectedComparePriceType == 'index_index' || this.selectedComparePriceType == 'index_currency')){
                this.selectedIndex1 = that.indices.filter(function(el) { return el.name == decodeURIComponent(val1) })[0]
            }
            if((this.selectedComparePriceType == 'index_index' || this.selectedComparePriceType == 'scheme_index')){
                this.selectedIndex2 = that.indices.filter(function(el) { return el.name == decodeURIComponent(val2) })[0]
            }
            if((this.selectedComparePriceType == 'currency_currency')){
                this.selectedCurrency1 = that.currencies.filter(function(el) { return el.cm_id == decodeURIComponent(val1) })[0]
            }
            if((this.selectedComparePriceType == 'currency_currency' || this.selectedComparePriceType == 'index_currency' || this.selectedComparePriceType == 'scheme_currency')){
                this.selectedCurrency2 = that.currencies.filter(function(el) { return el.cm_id == decodeURIComponent(val2) })[0]
            }
            that.priceCompare()
        }
        return true
        ...
        if(compare_ratio_type && val1 && val2){
            ...
            this.selectedFund1Ratio = that.funds.filter(function(el) { return el.fund_code == decodeURIComponent(val1) })[0]
            this.selectedFund2Ratio = that.funds.filter(function(el) { return el.fund_code == decodeURIComponent(val2) })[0]
        ...
        if(compare_composition_type && val1 && val2){
            ...
            this.selectedFund1Composition = that.funds.filter(function(el) { return el.fund_code == decodeURIComponent(val1) })[0]
            this.selectedFund2Composition = that.funds.filter(function(el) { return el.fund_code == decodeURIComponent(val2) })[0]
    ...
    CanvasJS.addColorSet("greenShades",['#6ab130','#fff']);
