<template>
    <div class="classifications-pg bg-gry">
        <div class="container">
            <div class="row align-items-center fund-port-cols">
                <div class="col-lg-6 col-md-6 col-sm-12 choose-classy select2-styles pl-0 fund-port-col-1">
                ...
                <div class="col-lg-6 col-md-6 col-sm-12 choose-classy select2-styles pr-0 fund-port-col-2">
                ...
                <div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12 perform-pmtr-lumpsum">
                        <div class="dy-table-wrap">
                            <div class="dy-table-block br-5 total-table">
                                <CustomTable
                                id="fund-portfolio"
                                :columns="columns"
                                :rows="portfolio"
                                default_sort_key="script_name"
...
export default {
  ...
  methods: {
   ...
   async getFundPortfolio(){
       let that = this
       that.process = true
       await axios.get('/api/v1/fund-portfolio', { params: {fund_code:that.selectedFund.fund_code} })
            .then(response => {
  ...
  mounted() {
    ...
    myPromise.then( async (resolve, reject) => {
        if(fund_house){
            ...
            await that.getFunds({fund_house:that.selectedFundHouse.fund_house})
        }
        return true
    }).then( async (resolve, reject) => {
        
        if(fund_code){
            that.selectedFund = that.funds.filter(function(el) { return el.fund_code == fund_code })[0]
        }
        return true
    })
  },
