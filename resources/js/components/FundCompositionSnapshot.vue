<template>
    <div class="custom-banner no-bg fw-banner" :class="{ 'monthly-ranking': !page_image }"
                ...
                <div class="col-md-12">
                    <div class="info_monitor_inner">
                        <div class="info_monitor_inner_wrapper mb-3">
                            <div
                                class="monthly_ranking_title d-block d-sm-flex align-items-center justify-content-between mb-3">
                            ...
                            <div class="monthly_ranking_Search_part mb-1">
                                <div class="row">
                                    <div class="col-lg-6 col-md-6 col-sm-12">
                                        ...
                                        <multiselect :disabled="loading || process" class="" label="name"
                                            track-by="ft_id" v-model="selectedFundClassification" tag-placeholder=""
                                            placeholder="Select Fund Classification" :options="fundClassifications"
                                            :multiple="false" :taggable="false" selectLabel="" :searchable="true"
                                ...
                                <div class="datatable_ll main_trer">
                                    <div class="table-responsive">
                                        ...
                                        <table id="example" class="table table-striped" style="width:100%" role="grid">
                                            <thead class="">
                                                <tr>
                                                    <th style="background-color: #222222 !important" colspan="4"
                                                        rowspan="1"></th>
                                                    <th style="background-color: #222222 !important" colspan="4"
                                                        rowspan="1">Equity</th>
                                                    <th style="background-color: #222222 !important" colspan="1"
                                                        rowspan="1"></th>
                                                </tr>
                                                <tr>
                                                    <th class="sorting" v-on:click="sortTable('fund_name')"
                                                        ...
                                                        width="20%">Name of the Fund</th>
                                                    <th class="sorting" v-on:click="sortTable('cash')"
                                                        :class="{ 'sorting_asc': sortKey == 'cash' && ascending, 'sorting_desc': sortKey == 'cash' && !ascending }"
                                                        tabindex="0" aria-controls="data-table" rowspan="1" colspan="1"
                                                        aria-label="Cash&amp;nbsp;%: activate to sort column ascending"
                                                        width="9%">Cash&nbsp;%</th>
                                                    <th class="sorting" v-on:click="sortTable('sov')"
                                                        :class="{ 'sorting_asc': sortKey == 'sov' && ascending, 'sorting_desc': sortKey == 'sov' && !ascending }"
                                                        tabindex="0" aria-controls="data-table" rowspan="1" colspan="1"
                                                        aria-label="Sov&amp;nbsp;%: activate to sort column ascending"
                                                        width="9%">Sov&nbsp;%</th>
                                                    <th class="sorting" v-on:click="sortTable('debt')"
                                                        :class="{ 'sorting_asc': sortKey == 'debt' && ascending, 'sorting_desc': sortKey == 'debt' && !ascending }"
                                                        tabindex="0" aria-controls="data-table" rowspan="1" colspan="1"
                                                        aria-label="Corp Debt&amp;nbsp;%: activate to sort column ascending">
                                                        Corp Debt&nbsp;%</th>
                                                    <th class="sorting" v-on:click="sortTable('eq_small')"
                                                        :class="{ 'sorting_asc': sortKey == 'eq_small' && ascending, 'sorting_desc': sortKey == 'eq_small' && !ascending }"
                                                        tabindex="0" aria-controls="data-table" rowspan="1" colspan="1"
                                                        aria-label="Small Cap&amp;nbsp;%: activate to sort column ascending">
                                                        Small Cap&nbsp;%</th>
                                                    <th class="sorting" v-on:click="sortTable('eq_mid')"
                                                        :class="{ 'sorting_asc': sortKey == 'eq_mid' && ascending, 'sorting_desc': sortKey == 'eq_mid' && !ascending }"
                                                        tabindex="0" aria-controls="data-table" rowspan="1" colspan="1"
                                                        aria-label="Mid Cap&amp;nbsp;%: activate to sort column ascending">
                                                        Mid Cap&nbsp;%</th>
                                                    <th class="sorting" v-on:click="sortTable('eq_large')"
                                                        :class="{ 'sorting_asc': sortKey == 'eq_large' && ascending, 'sorting_desc': sortKey == 'eq_large' && !ascending }"
                                                        tabindex="0" aria-controls="data-table" rowspan="1" colspan="1"
                                                        aria-label="Large Cap&amp;nbsp;%: activate to sort column ascending">
                                                        Large Cap&nbsp;%</th>
                                                    <th class="sorting" v-on:click="sortTable('eq_very_large')"
                                                        :class="{ 'sorting_asc': sortKey == 'eq_very_large' && ascending, 'sorting_desc': sortKey == 'eq_very_large' && !ascending }"
                                                        tabindex="0" aria-controls="data-table" rowspan="1" colspan="1"
                                                        aria-label="Very  Large  Cap&amp;nbsp;%: activate to sort column ascending">
                                                        Very Large Cap&nbsp;%</th>
                                                    <th class="sorting" v-on:click="sortTable('wt_pe')"
                                                        :class="{ 'sorting_asc': sortKey == 'wt_pe' && ascending, 'sorting_desc': sortKey == 'wt_pe' && !ascending }"
                                                        tabindex="0" aria-controls="data-table" rowspan="1" colspan="1"
                                            ...
                                            <tbody v-if="!process">
                                                <tr class="odd" role="row"
                                                    v-for="(composition, index) in composition_snapshot" :key="index">
                                                    ...
                                                    <td data-label="Very Large Cap %">{{ composition.eq_very_large }}
                                                    </td>
                                        ...
                                        <div class="table-total text-right" v-if="totalAmount">
                                            <p>Total Amount (Rs. in lacs) <span
                                                    class="total-amount">{{ totalAmount }}/-</span></p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="classification-disc" v-if="!process">
                                ...
                                <ul>
                                    ...
                                    <li>Loss Making Scrips have not been taken into account for calculation of total
                                        fund portfolio weighted PE.</li>
                                </ul>
                            </div>
                        </div>
                    </div>
        ...
        async getFundCompositionSnapshot() {
            ...
            await axios.get(that.app_url + '/api/v1/fund-composition-snapshot/' + that.selectedFundClassification.ft_id)
                ...
                .then(response => {
                    ...
                    that.for_text = 'For The Month of  ' + formattedMonth + ' , ' + first_fund.yearinfo;
                    that.snapshotText = that.selectedFundClassification.name + ' : ' + that.for_text
                })
                .catch(error => {
                    console.log(error);
                })
