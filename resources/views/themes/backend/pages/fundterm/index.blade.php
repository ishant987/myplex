...
@section('breadcrumb')
{{ Breadcrumbs::render('fundterm.index')  }} 
@endsection
  ...
  <div class="col-sm-12">
    <div class="card m-b-0">
      <div class="card-header">
        <h5>{{ __('admin.fundterm.all_txt') }}</h5>
        @if($roleRights['add'])
          <!-- Add New -->
          <x-link_add_new url="{{ route('admin.fundterm.create') }}" />
        @endif
      ...
      <div class="card-block">
       ...
       <form id="listDataForm" name="listDataForm" method="post" action="{{ route('admin.fundterm.delete') }}">
        {{ csrf_field() }}
        <div class="table-responsive dt-responsive">
          <table id="dom-jqry" class="table table-striped table-bordered nowrap">
            <thead>
              <tr>
                @if($roleRights['delete'])
                  ...
                  @endif    
                @endif 
                <th>{{ __('admin.fundterm.term_txt') }}</th>
                <th class="cc-w-95">{{ __('admin.fundterm.days_txt') }}</th>
            ...
            @if( count($dataListModel) > 0 )         
              ...
              <tr role="row" class="">
                @if($roleRights['delete'])
                  <td class="sorting_{{ $record->ftm_id }}">
                    <x-form.field.checkbox style="default" name="checkbox[]" value="{{ $record->ftm_id }}" fldclass="del-chkbx" />
                  </td>
                @endif
              <td>
                @if($roleRights['edit'])
                  <x-link_tooltip url="{{ route('admin.fundterm.edit', $record->ftm_id) }}" title="{{ $listDataAtrArr['edit_txt'] }}">
                    {{ $record->term }}
                  </x-link_tooltip>
                @else
                    {{ $record->term }}
