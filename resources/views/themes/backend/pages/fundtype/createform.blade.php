...
@section('breadcrumb')
{{ Breadcrumbs::render('fundtype.create') }}
@endsection
   ...
   <div class="col-sm-12">
      <div class="card m-b-0">
         <div class="card-header">
            <h5 class="card-header-text">{{ __('admin.fundtype.add_txt') }}</h5>
         </div>
         <div class="card-block">
            <x-form.alert type="{{ session()->get('alert') }}" title="{{ session()->get('title') }}" message="{{ session()->get('message') }}" />

            <form name="aeDataFrm" id="aeDataFrm" action="{{ route('admin.fundtype.store')}}" method="POST">
               {{ csrf_field() }}
               ...
               <x-form.group_lyt1_2_10 label="{{ __('admin.fundtype.active_passive_txt') }}" for="active_passive" error="{{ $errors->first('active_passive') }}" required="true">
                  <select name="active_passive" id="active_passive" class="form-control">
                     ...
                     <option value="{{ $key }}" @if( $key==old('active_passive') ) {{ 'selected' }} @endif>{{ $activePassive }}</option>
                     @endforeach
                  </select>
               </x-form.group_lyt1_2_10>
               ...
               <x-form.group_lyt1_2_10 label="{{ __('admin.fundtype.monthly_performance_txt') }}" for="monthly_performance" error="{{ $errors->first('monthly_performance') }}" required="true">
                  <select name="monthly_performance" id="monthly_performance" class="form-control">
                     ...
                     <option value="{{ $key }}" @if( $key==old('monthly_performance') ) {{ 'selected' }} @endif>{{ $monthlyPerformance }}</option>
                     @endforeach
                  </select>
               </x-form.group_lyt1_2_10>
