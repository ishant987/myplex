  ...
  <div class="col-sm-12">
    <div class="card m-b-0">
      ...
      <div class="card-block">
        ...
        <form name="eDataFrm" id="eDataFrm" action="{{ route('admin.topic.update', $dataArr->aet_id) }}" method="POST">
          {{ csrf_field() }}
          {{ method_field('PATCH') }}
          ...
          <x-form.group_lyt1_2_10 label="{{ __('admin.parent_txt') }}" for="role" error="{{ $errors->first('parent') }}">
            <select id="parent" class="form-control" name="parent">
              ...
              <option value="{{ $value }}" @if((old('parent')> 0) && ($value == old('parent'))) {{ 'selected' }} @elseif((($dataArr->parent > 0) && ($value == $dataArr->parent))) {{ 'selected' }} @endif>{{ $optVal }}</option>
              @endforeach
            </select>
          </x-form.group_lyt1_2_10>
          ...
          <div class="row m-b-20{{ $errors->first('assigned_to')?' has-danger':'' }}">
            ...
            <div class="col-sm-5">
              ...
              <select name="assigned_to[]" id="multiselect_to" class="form-control multiselect" size="15" multiple="multiple">
                @if(count($toObj) > 0)
                @foreach( $toObj as $key => $value )
                <option value="{{ $value->u_id }}">{{ rtrim($value->users->f_name.' '.$value->users->l_name).' ('.$value->users->email.')' }}</option>
                @endforeach
                @endif
              </select>
            </div>
