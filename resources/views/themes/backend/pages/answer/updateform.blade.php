...
<div class="row">
 <div class="col-sm-12">
  ...
  <div class="card-block">
    ...
    <form name="eDataFrm" id="eDataFrm" action="{{ route('admin.answer.update', $dataArr->aeqa_id) }}" method="POST">
      {{ csrf_field() }}
      {{ method_field('PATCH') }}

      <x-form.group_lyt1_2_10 label="{{ __('askexpert.answer.label_txt') }}" for="title" error="{{ $errors->first('title') }}" required="true" info="{!! __('admin.info.descp') !!}">
      <x-form.field.textarea id="answer" name="answer" value="{!! $dataArr->answer !!}" rows="8" />
