...
@section('breadcrumb')
{{ Breadcrumbs::render('fundtaxation.index') }}
@endsection
  ...
  <div class="col-sm-12">
    <div class="card m-b-0">
      <div class="card-header">
        <h5>{{ __('admin.fundtaxation.all_txt') }}</h5>
        @if($roleRights['add'])
        <!-- Add New -->
        <x-link_add_new url="{{ route('admin.fund-taxation.create') }}" />
        @endif
      ...
      <div class="card-block">
        ...
        <form id="listDataForm" name="listDataForm" method="post" action="{{ route('admin.fund-taxation.delete') }}">
          {{ csrf_field() }}
            ...
            <table id="dom-jqry" class="table table-striped table-bordered nowrap">
              ...
              <tbody>
                ...
                <tr role="row" class="">
                  ...
                  <td>
                    @if($roleRights['edit'])
                    <x-link_tooltip url="{{ route('admin.fund-taxation.edit', $record->ft_id) }}" title="{{ $listDataAtrArr['edit_txt'] }}">
                      {{ $record->title }}
                    </x-link_tooltip>
                    @else
                    {{ $record->title }}
                  ...
                  <td>
                    @if($roleRights['edit'])
                    <label id="change_status{{ $record->ft_id }}" onclick="return changeStatus('ft_id', {{ $record->ft_id }}, 'fund_taxation', {{ $record->status }}, '{{ $statusAtrArr['status_type'] }}');" class="label btn-{{ $listDataAtrArr['alert_css'][$record->status] }}">{{ $statusAtrArr['label'][$record->status] }}</label>
                    @else
