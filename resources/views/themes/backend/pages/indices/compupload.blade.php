...
@section('breadcrumb')
{{ Breadcrumbs::render('indices-comp.values.create') }}
@endsection
   ...
   <div class="col-sm-12">
      <div class="card m-b-0">
         <div class="card-header">
            <h5 class="card-header-text">{{ __('admin.indices.comp.upld_lbl_txt') }}</h5>
         </div>
         <div class="card-block">
            <x-form.alert type="{{ session()->get('alert') }}" title="{{ session()->get('title') }}" message="{{ session()->get('message') }}" />
               <form id="upload_bulk_sheet_form" name="upload_bulk_sheet_form" method="post" action="@if($msg=='error'){{route('admin.indices-comp.values.store', $msg)}}@else{{route('admin.indices-comp.values.store')}}@endif" enctype="multipart/form-data">
               {{ csrf_field() }}
               ...
               <x-form.group_lyt1_2_10 label="{{ __('admin.file_txt') }}" for="file_upload" error="{{ $errors->first('file_upload') }}" info="{!! __('admin.indices.comp.file_format_info') !!}" required="true">
                  <x-form.field.file id="file_upload" name="file_upload" />
                  <div class="m-t-10 has-primary">
                     <span class="col-form-label info">
                        <x-link url="{{ asset('themes/backend/assets/formats/upload-monthly-indices-composition-value.csv') }}" class="b-b-primary text-primary" target="_blank">{{ __('common.click_here_txt') }}</x-link>{{ __('admin.bulk_upload.dwnld_csv_file_format_info') }}
                     </span>
                  </div>
               </x-form.group_lyt1_2_10>
               <div class="row">
                  <div class="col-sm-12">
                     <x-form.group_lyt1_2_10 class="m-t-10">
                        ...
                        <x-form.field.button type="reset" id="cancel" name="cancel" class="btn-danger" text="{{ __('admin.cancel_txt') }}" onclick="location.href = '{{route('admin.indices-comp.values.create')}}';" />
                        @if($msg=='error')
