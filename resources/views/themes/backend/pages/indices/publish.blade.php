...
@section('breadcrumb')
{{ Breadcrumbs::render('indices.publish.create') }}
@endsection
  ...
  <div class="col-sm-12">
    <div class="card m-b-0">
      <div class="card-header">
        <h5 class="card-header-text">{{ __('admin.indices.publish_label_txt') }}</h5>
      </div>
      <div class="card-block">
        <x-form.alert type="{{ session()->get('alert') }}" title="{{ session()->get('title') }}" message="{{ session()->get('message') }}" />

        <form name="addForm" id="addForm" action="{{ route('admin.indices.publish.store')}}" method="POST">
          {{ csrf_field() }}
