   ...
   <div class="col-sm-12">
      <div class="card m-b-0">
         ...
         <div class="card-block">
            ...
            <form name="eDataFrm" id="eDataFrm" action="{{ route('admin.usergroup.update', $dataArr->u_g_id) }}" method="POST">
               {{ csrf_field() }}
               {{ method_field('PATCH') }}

               <x-form.group_lyt1_2_10 label="{{ __('subscribeduser.user_group.name') }}" for="group_name" error="{{ $errors->first('group_name') }}" required="true">
                  <x-form.field.text id="group_name" name="group_name" value="{{ $dataArr->group_name }}" />
               ...
               <div class="row m-b-20{{ $errors->first('assigned_to')?' has-danger':'' }}">
                  ...
                  <div class="col-sm-5">
                     ...
                     <select name="assigned_to[]" id="multiselect_to" class="form-control multiselect" size="25" multiple="multiple">
                        @foreach( $slctdSbsUsrObj as $key => $value )
                        <option value="{{ $value->u_id }}">{{ $value->user->fullinfo }}</option>
                        @endforeach
                     </select>
                  </div>
