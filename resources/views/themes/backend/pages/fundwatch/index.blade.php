          ...
          <div class="table-responsive dt-responsive">
            <table id="dom-jqry" class="table table-striped table-bordered nowrap">
              ...
              <tbody>
                ...
                <tr role="row" class="">
                  @if($roleRights['delete'])
                  <td class="sorting_{{ $record->fw_id }}">
                    <x-form.field.checkbox style="default" name="checkbox[]" value="{{ $record->fw_id }}" fldclass="del-chkbx" />
                  </td>
                  @endif
                  <td>
                    @if($roleRights['edit'])
                    <x-link_tooltip url="{{ route('admin.fund-watch.edit', $record->fw_id) }}" title="{{ $listDataAtrArr['edit_txt'] }}">
                      {{ $record->title }}
                    </x-link_tooltip>
                    @else
                    {{ $record->title }}
                  ...
                  <td>
                    @if($roleRights['edit'])
                    <label id="change_status{{ $record->fw_id }}" onclick="return changeStatus('fw_id', {{ $record->fw_id }}, 'fund_watch', {{ $record->status }}, '{{ $statusAtrArr['status_type'] }}');" class="label btn-{{ $listDataAtrArr['alert_css'][$record->status] }}">{{ $statusAtrArr['label'][$record->status] }}</label>
                    @else
