...
@section('breadcrumb')
{{ Breadcrumbs::render('fundclassification.index') }}
@endsection
  ...
  <div class="col-sm-12">
    <div class="card m-b-0">
      <div class="card-header">
        <h5>{{ __('admin.fundclassification.all_txt') }}</h5>
        @if($roleRights['add'])
        <!-- Add New -->
        <x-link_add_new url="{{ route('admin.fund-classification.create') }}" />
        @endif
      ...
      <div class="card-block">
        ...
        <form id="listDataForm" name="listDataForm" method="post" action="{{ route('admin.fund-classification.delete') }}">
          {{ csrf_field() }}
          <div class="table-responsive dt-responsive">
            <table id="dom-jqry" class="table table-striped table-bordered nowrap">
              ...
              <tbody>
                ...
                <tr role="row" class="">
                  @if($roleRights['delete'])
                  <td class="sorting_{{ $record->fc_id }}">
                    <x-form.field.checkbox style="default" name="checkbox[]" value="{{ $record->fc_id }}" fldclass="del-chkbx" />
                  </td>
                  @endif
                  <td>
                    @if($roleRights['edit'])
                    <x-link_tooltip url="{{ route('admin.fund-classification.edit', $record->fc_id) }}" title="{{ $listDataAtrArr['edit_txt'] }}">
                      {{ $record->title }}
                    </x-link_tooltip>
                    @else
                    {{ $record->title }}
                  ...
                  <td>
                    @if($roleRights['edit'])
                    <label id="change_status{{ $record->fc_id }}" onclick="return changeStatus('fc_id', {{ $record->fc_id }}, 'fund_classification', {{ $record->status }}, '{{ $statusAtrArr['status_type'] }}');" class="label btn-{{ $listDataAtrArr['alert_css'][$record->status] }}">{{ $statusAtrArr['label'][$record->status] }}</label>
                    @else
