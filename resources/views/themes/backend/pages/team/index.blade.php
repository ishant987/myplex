  ...
  <div class="col-sm-12">
    <div class="card m-b-0">
      <div class="card-header">
        <h5>{{ __('admin.team.all_txt') }}</h5>
        @if($roleRights['add'])
        <!-- Add New -->
        <x-link_add_new url="{{ route('admin.team.create') }}" />
        @endif
      ...
      <div class="card-block">
        ...
        <form id="listDataForm" name="listDataForm" method="post" action="{{ route('admin.team.delete') }}">
          {{ csrf_field() }}
          <div class="table-responsive dt-responsive">
            <table id="dom-jqry" class="table table-striped table-bordered nowrap">
              ...
              <tbody>
                ...
                <tr role="row" class="">
                  @if($roleRights['delete'])
                  <td class="sorting_{{ $record->team_id }}">
                    <x-form.field.checkbox style="default" name="checkbox[]" value="{{ $record->team_id }}" fldclass="del-chkbx" />
                  </td>
                  @endif
                  <td>
                    @if($roleRights['edit'])
                    <x-link_tooltip url="{{ route('admin.team.edit', $record->team_id) }}" title="{{ $listDataAtrArr['edit_txt'] }}">
                      {{ $record->name }}
                    </x-link_tooltip>
                    @else
                    {{ $record->name }}
                  ...
                  <td>
                    @if($roleRights['edit'])
                    <label id="change_status{{ $record->team_id }}" onclick="return changeStatus('team_id', {{ $record->team_id }}, 'teams', {{ $record->status }}, '{{ $statusAtrArr['status_type'] }}');" class="label btn-{{ $listDataAtrArr['alert_css'][$record->status] }}">{{ $statusAtrArr['label'][$record->status] }}</label>
                    @else
