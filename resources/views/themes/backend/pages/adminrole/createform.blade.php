...
@section('breadcrumb')
{{ Breadcrumbs::render('adminrole.create') }} 
@endsection
   ...
   <div class="col-sm-12">
      <div class="card m-b-0">
         <div class="card-header">
            <h5 class="card-header-text">{{ __('admin.add_admnrole_user_txt') }}</h5>
         </div>
         <div class="card-block">
            <x-form.alert type="{{ session()->get('alert') }}" title="{{ session()->get('title') }}" message="{{ session()->get('message') }}" />
            
            <form name="aeUsrFrm" id="aeUsrFrm" action="{{ route('admin.adminrole.store')}}" method="POST">
              {{ csrf_field() }}
                    ...
                    <table class="table table-bordered m-b-0 user-role-table">
                       ...
                       <tbody>
                        @foreach($modules as $key=>$module)  
                          @if($module->getModuleMethods() && count($module->getModuleMethods())>0)
                           <tr class="{{ $maxTdCnt }}">
                              <td>
                              ...
                              @foreach($module->getModuleMethods() as $key2=>$method)
                              <td @if( $maxTdCnt !=count($module->getModuleMethods()) && $key2==count($module->getModuleMethods())-1 ) colspan="{{ $maxTdCnt-$key2 }}" @endif class="user-method-perm ">
                                ...
                                <div class="checkbox-fade has-primary fade-in-default">
                                   <label>
                                    @if($method->default_present == 0)
                                      <input type="checkbox" name="method[]" value="{{ $method->method_id }}">
                                      <span class="cr">
                                      <i class="cr-icon icofont icofont-ui-check txt-default"></i>
                                     </span>
                                    @else
