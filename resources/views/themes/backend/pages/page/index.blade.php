  ...
  <div class="col-sm-12">
    <div class="card m-b-0">
      <div class="card-header">
        ...
        @if($roleRights['add'])
          <!-- Add New -->
          <x-link_add_new url="{{ route('admin.page.create.custom') }}" />
        @endif
      ...
      <div class="card-block">
       ...
       <form id="listDataForm" name="listDataForm" method="post" action="{{ route('admin.deletepage') }}">
        {{ csrf_field() }}
          ...
          <table id="dom-jqry" class="table table-striped table-bordered nowrap">
            <thead>
              <tr>
                ...
                @endif
                <th class="cc-w-150">{{ __('admin.template_txt') }}</th>
            ...
            @if( count($dataListModel) > 0 )       
              ...
              <tr role="row" class="">
                @if($roleRights['delete'])
                  <td class="sorting_{{ $record->page_id }}">
                    <x-form.field.checkbox style="default" name="checkbox[]" value="{{ $record->page_id }}" fldclass="del-chkbx" />
                  </td>
                @endif
              <td>
                @if($roleRights['edit'])
                  <x-link_tooltip url="{{ route('admin.page.edit.custom', [$record->page_id, $record->template_id]) }}" title="{{ $record->note != '' ? $record->note : $listDataAtrArr['edit_txt'] }}">
                    {{ $record->title }}
                  </x-link_tooltip>
                @else
                    {{ $record->title }}
                ...
                @endif
                @if( $moduleAtrArr['flds_hide']['parent'] == $moduleAtrArr['bool_false'] )
                  <td>{{ $record->parentpage != null ? $record->parentpage->title : '' }}</td>
                @endif
                <td>{{ $record->template != null ? $record->template->title : '' }}</td>
                @if( $moduleAtrArr['flds_hide']['c_order'] == $moduleAtrArr['bool_false'] )
                  <td>{{ $record->c_order > 0 ? $record->c_order : '' }}</td>
                @endif
                <td>
                  @if($roleRights['edit'])
                    <label id="change_status{{ $record->page_id }}" onclick="return changeStatus('page_id', {{ $record->page_id }}, 'pages', {{ $record->status }}, '{{ $statusAtrArr['status_type'] }}');" class="label btn-{{ $listDataAtrArr['alert_css'][$record->status] }}">{{ $statusAtrArr['label'][$record->status] }}</label>
                  @else
                    <label id="change_status{{ $record->page_id }}" class="label btn-{{ $listDataAtrArr['alert_css'][$record->status] }} no-drop">{{ $statusAtrArr['label'][$record->status] }}</label>
                  @endif                                   
                </td>
