...
@section('breadcrumb')
{{ Breadcrumbs::render('knowtheratio.index')  }}
@endsection
  ...
  <div class="col-sm-12">
    <div class="card m-b-0">
      <div class="card-header">
        <h5>{{ __('admin.knowtheratio.all_txt') }}</h5>
        @if($roleRights['add'])
        <!-- Add New -->
        <x-link_add_new url="{{ route('admin.know-the-ratio.create') }}" />
        @endif
      ...
      <div class="card-block">
        ...
        <form id="listDataForm" name="listDataForm" method="post" action="{{ route('admin.know-the-ratio.delete') }}">
          {{ csrf_field() }}
          <div class="table-responsive dt-responsive">
            <table id="dom-jqry" class="table table-striped table-bordered nowrap">
              ...
              <tbody>
                ...
                <tr role="row" class="">
                  @if($roleRights['delete'])
                  <td class="sorting_{{ $record->ktr_id }}">
                    <x-form.field.checkbox style="default" name="checkbox[]" value="{{ $record->ktr_id }}" fldclass="del-chkbx" />
                  </td>
                  @endif
                  <td>
                    @if($roleRights['edit'])
                    <x-link_tooltip url="{{ route('admin.know-the-ratio.edit', $record->ktr_id) }}" title="{{ $listDataAtrArr['edit_txt'] }}">
                      {{ $record->title }}
                    </x-link_tooltip>
                    @else
                    {{ $record->title }}
                  ...
                  <td>
                    @if($roleRights['edit'])
                    <label id="change_status{{ $record->ktr_id }}" onclick="return changeStatus('ktr_id', {{ $record->ktr_id }}, 'know_the_ratio', {{ $record->status }}, '{{ $statusAtrArr['status_type'] }}');" class="label btn-{{ $listDataAtrArr['alert_css'][$record->status] }}">{{ $statusAtrArr['label'][$record->status] }}</label>
                    @else
