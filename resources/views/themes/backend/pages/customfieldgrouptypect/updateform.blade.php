...
@section('breadcrumb')
{{ Breadcrumbs::render('customfield.grouptype.classtemplate.edit', $dataArr) }} 
@endsection
    ...
    <div class="col-sm-12">
        <div class="card m-b-0">
            <div class="card-header">
                <h5 class="card-header-text">{{ __('admin.customfield.grouptype.classtemplate.edit_txt') }}</h5>
            </div>
            <div class="card-block">
            <x-form.alert type="{{ session()->get('alert') }}" title="{{ session()->get('title') }}" message="{{ session()->get('message') }}" />
            
            <form name="aeDataFrm" id="aeDataFrm" action="{{ route('admin.customfield.grouptype.classtemplate.update', [$dataArr->cfgt->cf_group_id, $dataArr->cf_group_type_id, $dataArr->cf_gt_class_template_id])}}" method="POST">
                {{ csrf_field() }}
                {{ method_field('PATCH') }}
                ...
                <select id="class_id" class="form-control" name="class_id">
                    ...
                    <option value="{{ $class_id }}" @if((old('class_id') > 0) && ($class_id == old('class_id'))) {{ 'selected' }} @elseif((($dataArr->class_id) && ($class_id == $dataArr->class_id))) {{ 'selected' }} @endif>{{ $class_name }}</option>
                    @endforeach
                </select>
                ...
                <select id="template_id" class="form-control" name="template_id">
                    @foreach( $classTemplatesAssoc as $template_id => $template_name )
                        <option value="{{ $template_id }}" @if((old('template_id') > 0) && ($template_id == old('template_id'))) {{ 'selected' }} @elseif((($dataArr->template_id) && ($template_id == $dataArr->class_id))) {{ 'selected' }} @endif>{{ $template_name }}</option>
                    @endforeach
                </select>
                </x-form.group_lyt1_2_10>
...
<script>
    ...
    $(document).ready(function () {
        getclasstemplate('{{ old('template_id')??$dataArr->template_id }}');
    });
