  ...
  <div class="col-sm-12">
    <div class="card m-b-0">
      <div class="card-header">
        <h5>{{ __('admin.fund.aums.list_label_txt') }}</h5>
        <div class="c-f-btns">
          <div class="c-f-b-f">
            <!-- Filter -->
            <x-form.btn_filter />
          ...
          <div class="c-f-b-r">
            ...
            <form id="listDataForm" name="listDataForm" method="post" action="{{ route('admin.aums.delete') }}">
              {{ csrf_field() }}
      ...
      <div class="card-block">
        ...
        <div class="dataTables_wrapper dt-bootstrap4 no-footer">
          <form id="filterForm" name="filterForm" method="get" action="{{ route('admin.aums.list') }}">
          ...
          <table class="table table-striped table-bordered nowrap">
            <thead>
              <tr>
                ...
                <th class="cc-w-125">{{ $sortbyArr['corpus_entry'] }}</th>
                <th class="cc-w-125">{{ $sortbyArr['percentage_change'] }}</th>
                <th class="cc-w-80">{{ $sortbyArr['corpus_change'] }}</th>
            ...
            <tbody>
              ...
              <tr role="row" class="">
                ...
                <td>{{ $record->corpus_entry }}</td>
                <td>{{ $record->percentage_change }}</td>
                <td>{{ $record->corpus_change }}</td>
...
<script>
  ...
  function filter() {
    ...
    window.location.href = "{{ route('admin.aums.list') }}" + "?page=0" + searchPagiData + "&ppage=" + "{{ $perPage }}";
    return false;
  }
  /*Reset Filter*/
  function resetfilter() {
    window.location.href = "{{ route('admin.aums.list') }}";
  }
</script>
@endpush