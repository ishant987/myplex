  ...
  <div class="col-sm-12">
    <div class="card m-b-0">
      ...
      <div class="card-block">
        ...
        <form id="listDataForm" name="listDataForm" method="post" action="{{ route('admin.navs.values.update') }}">
          {{ csrf_field() }}
            ...
            <table id="dom-jqry-all" class="table table-striped table-bordered nowrap">
              <thead>
                <tr>
                  ...
                  <th>{{ __('admin.fund.corelation_txt') }}</th>
                  <th class="cc-w-150">{{ __('admin.fund.code_txt') }}</th>
                  <th class="cc-w-95">{{ __('admin.fund.nav.label_readonly_txt') }}</th>
                  <th class="cc-w-95 no-sort">{{ __('admin.fund.nav.label_txt') }}</th>
                </tr>
              </thead>
              <tbody>
                @if( count($dataListModel) > 0 )
                @foreach( $dataListModel as $key => $record )
                <tr role="row" class="{{ $record->last_nav == 'NA'?'no-record':'' }}">
                  <td>{{ $record->fund_name }}</td>
                  <td>{{ $record->cor }}</td>
                  <td>{{ $record->fund_code }}</td>
                  <td>{{ $record->last_nav != '' ? $record->last_nav : 0 }}</td>
                  <td>
                    <x-form.field.text id="nav_{{ $loop->iteration }}" name="nav[{{ $record->fund_code }}]" value="{{ $record->last_nav != '' ? $record->last_nav : 0 }}" />
                  </td>
                </tr>
                @endforeach
                @else
