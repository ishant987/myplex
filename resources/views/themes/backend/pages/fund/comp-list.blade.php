...
@section('breadcrumb')
{{ Breadcrumbs::render('fund-comp.list')  }}
@endsection
    ...
    <div class="col-sm-12">
        <div class="card m-b-0">
            <div class="card-header">
                <h5>{{ __('admin.fund.comp.list_label_txt') }}</h5>
                <div class="c-f-btns">
                    <div class="c-f-b-f">
                        <!-- Filter -->
                        <x-form.btn_filter />
                    ...
                    <div class="c-f-b-r">
                        ...
                        <form id="listDataForm" name="listDataForm" method="post" action="{{ route('admin.fund-comp.delete') }}">
                            {{ csrf_field() }}
            ...
            <div class="card-block">
                ...
                <div class="dataTables_wrapper dt-bootstrap4 no-footer">
                    <form id="filterForm" name="filterForm" method="get" action="{{ route('admin.fund-comp.list') }}">
                    ...
                    <table class="table table-striped table-bordered nowrap">
                        <thead>
                            <tr>
                                ...
                                <th class="cc-w-125">{{ $sortbyArr['category'] }}</th>
                                <th class="cc-w-80">{{ $sortbyArr['content_per'] }}</th>
                                <th class="cc-w-80">{{ $sortbyArr['indices_per'] }}</th>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <x-form.field.text id="fcy" name="fcy" value="{{ $fltrDataArr['category'] ?? '' }}" />
                                </td>
                                <td></td>
                                <td></td>
                                <td></td>
                            ...
                            <tr role="row" class="">
                                ...
                                <td>{{ $record->scrip_name }}</td>
                                <td>{{ $record->industry }}</td>
                                <td>{{ $record->category }}</td>
                                <td>{{ $record->content_per }}</td>
                                <td>{{ $record->indices_per }}</td>
...
<script>
    ...
    function filter() {
        ...
        window.location.href = "{{ route('admin.fund-comp.list') }}" + "?page=0" + searchPagiData + "&ppage=" + "{{ $perPage }}";
        return false;
    }
    /*Reset Filter*/
    function resetfilter() {
        window.location.href = "{{ route('admin.fund-comp.list') }}";
    }
</script>
@endpush