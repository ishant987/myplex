        ...
        <div class="comp_schem_bdr">
            <div class="tab_snap_shot">
                <ul class="nav nav-pills mb-3 justify-content-md-center" id="pills-tab" role="tablist">
                    <li class="nav-item me-5" role="presentation"><a class="text-dark nav-link @if((isset($ratio_type) && ($ratio_type == 'returns')) || !isset($ratio_type)) active @endif " href="performance-synopsis?type=<?=$type?>&ratio_type=returns" ><i class="fa-solid fa-calendar-check"></i>By Ratio: Returns</a></li>
                    <li class="nav-item" role="presentation"><a class="text-dark nav-link @if((isset($ratio_type) && ($ratio_type == 'jensens_alpha')) || !isset($ratio_type)) active @endif  " href="performance-synopsis?type=<?=$type?>&ratio_type=jensens_alpha" ><i class="ph-calendar-check"></i>By Ratio: Jensen’s Alpha</a></li>
                </ul>
                <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
                    <li class="nav-item" role="presentation"><a class="text-dark nav-link @if((isset($type) && ($type == 'quartile-active')) || !isset($type)) active @endif " href="performance-synopsis?type=quartile-active&ratio_type=<?=$ratio_type?>" ><i class="fa-solid fa-calendar-check"></i> Quartile (Active Schemes)</a></li>
                    <li class="nav-item" role="presentation"><a class="text-dark nav-link @if((isset($type) && ($type == 'quartile-passive')) || !isset($type)) active @endif  " href="performance-synopsis?type=quartile-passive&ratio_type=<?=$ratio_type?>" ><i class="ph-calendar"></i> Quartile (Passive Schemes)</a></li>
                    <li class="nav-item" role="presentation"><a class="text-dark nav-link @if((isset($type) && ($type == 'decile-active')) || !isset($type)) active @endif " href="performance-synopsis?type=decile-active&ratio_type=<?=$ratio_type?>" ><i class="ph-calendar-check"></i> Decile (Active Schemes)</a></li>
                    <li class="nav-item" role="presentation"><a class="text-dark nav-link @if((isset($type) && ($type == 'decile-passive')) || !isset($type)) active @endif " href="performance-synopsis?type=decile-passive&ratio_type=<?=$ratio_type?>" ><i class="ph-calendar"></i> Decile (Passive Schemes)</a></li>
                </ul>
                        ...
                        <div class="top_table_bg_color mb-3 p-0" style="background: none;">
                            ...
                            <div class="fund_compo p-0" style="background: none;" id="pills-tabContent">          
                                ...
                                <div class="w-100 text-center single_fund titleBox d-none">
                                    ...
                                    <h4>
                                        @if($type=='quartile-passive')
                                            Comprehensive Report of Quartile Rank Improvement by Fund Houses
                                        @elseif($type=='decile-active')
                                            Comprehensive Report of Decile Rank Improvement by Fund Houses
                                        @elseif($type=='decile-passive')
                                            Comprehensive Report of Decile Rank Improvement by Fund Houses
                                        @else
                                            Comprehensive Report of Quartile Rank Improvement by Fund Houses
                                        @endif
                                    </h4>                                    
                                    <h5 class="text-white">{{ date("F Y",strtotime($monthly_performance_synopses_last_date.' -1 year first day of next month')) }} - {{ date("F Y",strtotime($monthly_performance_synopses_last_date)) }}</h5>
                                </div>
                                <div class="titleBox2 fw-bold" style="border: 1px solid #fff; border-radius: 18px; padding: 10px;background: var(--darkgreen);">
                                    ...
                                    <!-- {{ date("jS F'Y",strtotime($monthly_performance_synopses_last_date.' -1 year first day of next month')) }} -->
                                    <h5>{{ date("F Y",strtotime($monthly_performance_synopses_last_date.' -1 year first day of next month')) }} - {{ date("F Y",strtotime($monthly_performance_synopses_last_date)) }}</h5>
                                    
                                </div>
                                <div class="fund_compo_in " id="fund_compo_in">
                                    @if($type=='quartile-passive' || $type=='quartile-active')
                                        ...
                                        <table  class="table cmn_table port_up_table" width="100%">
                                            <thead>
                                                ...
                                                <tr>
                                                @for($i=4;$i>=1;$i--)
                                                    <th scope="col" class="text-center" style="background-color: #00665E !important;border:none;border-left:1px solid #fff !important;border-top:1px solid #fff !important;border-bottom:1px solid #fff !important;"> {{ $i}} <span class="filter__arrow"><a href="javascript:void(0)"><i class="ph-arrows-down-up-bold"></i></a></span>
                                                    </th>
                                            ...
                                            <tbody id="" class="fs-8">
                                                @if(!empty($performance_synopses))
                                                @foreach($performance_synopses as $k=>$performance_synopsis)
                                                    
                                                <tr>
                                                    <td scope="col" class="text-center ">{{ $performance_synopsis['total_schemes'] }}</td>
                                                    <td scope="col" class="text-start" style="width:175px!important;">{{ $performance_synopsis['fund_house'] }}</td>
                                                    @for($i=4;$i>=1;$i--)
                                                        <td scope="col" class="text-center cursor-pointer" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-html="true" title="{{ $performance_synopsis['quartileSchemes'.$i] }}" style="border-left:1px solid #000 !important;">
                                                            {{ $performance_synopsis['quartile'.$i] }} 
                                                        </td>
                                                        <td scope="col" class="text-center"  style="background-color:#ADC178;border-right:1px solid #000 !important;">
                                                            
                                                            @if($performance_synopsis['quartile'.$i]-$performance_synopsis['_quartile'.$i] > 0)
                                                                {{ $performance_synopsis['quartile'.$i]-$performance_synopsis['_quartile'.$i] }}
                                                            <i class="fa-solid fa-arrow-up text-success"></i>
                                                            @elseif($performance_synopsis['quartile'.$i]-$performance_synopsis['_quartile'.$i] < 0)
                                                                {{ ($performance_synopsis['quartile'.$i]-$performance_synopsis['_quartile'.$i]) * -1  }}
                                    ...
                                    @else
                                        ...
                                        <table id="" class="table cmn_table port_up_table" style="width:100%">
                                            <thead>
                                                ...
                                                <tr>
                                                    @for($i=10;$i>=1;$i--)
                                                            <th scope="col" class="text-center" style="background-color: #00665E !important;">{{$i}} <span class="filter__arrow"><a href="javascript:void(0)"><i class="ph-arrows-down-up-bold"></i></a></span></th>
                                            ...
                                            <tbody class="fs-8">
                                                @if(!empty($performance_synopses))
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td scope="col" class="text-start" style="width:175px!important;">{{ $performance_synopsis['fund_house'] }}</td>
                                                        @for($i=10;$i>=1;$i--)
                                                        <td scope="col" class="text-center"  data-bs-toggle="tooltip" data-bs-html="true"  data-bs-placement="top" title="{{ $performance_synopsis['decileSchemes'.$i] }}" style="border-left:1px solid #000 !important;">
                                                            {{ $performance_synopsis['decile'.$i] }} 
                                                            
                                                            @if($performance_synopsis['decile'.$i]-$performance_synopsis['_decile'.$i] > 0)
                                                                ({{ $performance_synopsis['decile'.$i]-$performance_synopsis['_decile'.$i] }}
                                                            <i class="fa-solid fa-arrow-up text-success"></i>)
                                                            @elseif($performance_synopsis['decile'.$i]-$performance_synopsis['_decile'.$i] < 0)
                                                                ({{ ($performance_synopsis['decile'.$i]-$performance_synopsis['_decile'.$i]) * -1}}
                                                            <i class="fa-solid fa-arrow-down text-danger"></i>)
                                                            @else
                                                                ({{ $performance_synopsis['decile'.$i]-$performance_synopsis['_decile'.$i] }})
                                                            @endif
                                ...
                                <!-- <p class="discl"><strong>Disclaimer :</strong> {!! $disclaimer_text !!}</p> -->
