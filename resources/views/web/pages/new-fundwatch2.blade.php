...
<div id="banner" class="banner pdfPrint" style="background: url(https://myplexus.com/themes/frontend/assets/v1/img/inner_banner.png)">
    <div class="container">
        <h1>fund watch</h1>
        <h4>{{ $fundMaster->fund_name }}</h4>
        <h5 class="text-white">Date of posting : {{ date('d M Y',strtotime($fundWatch->published_date)) }}</h5>
    ...
    <div class="container">
        <div class="body_content">
            <div class="content_left pdfPrint" id="content_left">
                ...
                <h2>Preamble</h2>
                <p class="pre_text">{!! $fundWatch->preamble !!}</p>
                
                <div class="fund_detail mt_15">
                    <div class="single_fund">
                        <h3 style="line-height: 0px;" class="text-white">Fund Details</h3>
                        <ul style="margin-top: 20px;">
                            <li><p>Starting Date</p> <i>:</i> <span>{{$schemenamei}}</span></li>
                            <li><p>Fund Type</p> <i>:</i> <span>{{ $fundMaster->classification }}</span></li>
                            <li><p>Benchmark</p> <i>:</i> <span>{{ $fundMaster->indices_name }}</span></li>
                        </ul>
                    </div>
                    <div class="single_fund">
                        <h3 style="line-height: 0px;" class="text-white">Management Details</h3>
                        <ul style="margin-top: 20px;">
                            <li><p>Fund Manager</p> <i>:</i> <span>{{ $fundMaster->fund_manager }}</span></li>
                            <li><p>Schemes Managed</p> <i>:</i> <span>{{ $snm }}</span></li>
                            <li><p>Total Asset Size</p> <i>:</i> <span>{{ number_format($total_asset[0]->corpus_entry * 0.01, 2) }} Crores [ {{ date('jS F Y',strtotime($total_asset[0]->entry_date)); }}]</span></li>
                        </ul>
                    </div>
                </div>
                    ...
                    <div class="member_right text-white">
                        <ul>
                            ...
                            @foreach($teamArray as $teamMember)
                                <li><a href="#" class=" text-white">{{$i.'. '.$teamMember}}</a></li>
                                @php $i++; @endphp
                            @endforeach
                        </ul>                                      
                    </div>
                ...
                <div class="perform mt_15">
                        ...
                        <div class="perform_single">
                            ...
                            <table class="cmn_table ">
                                <thead>
                                    <tr style="border-width: 0;">
                                        <th></th>
                                        <th style="border-right: 1px solid;padding: -2px 16px;">{{ $return_date_6_month.' - '.$return_date }} (6M)</th>
                                        <th style="border-right: 1px solid;">{{ $return_date_1.' - '.$return_date }} (1Y)</th>
                                        <th style="border-right: 1px solid;">{{ $return_date_2.' - '.$return_date }} (2Y)</th>
                                        <th style="border-right: 1px solid;">{{ $return_date_3.' - '.$return_date }} (3Y)</th>
                                        <th>{{ $return_date_5.' - '.$return_date }} (5Y)</th>
                                    </tr>
                                </thead>
                                <tbody id="retunr_continus_table">
                                @forelse($returnContinious as $keyrc=>$returnContiniousData)
                                    <tr>
                                        <td data-label="Return">{{ $returnContiniousData->label}}</td>
                                        <td data-label="6M">{{ $returnContiniousData->SIXMONTHS !='' ? number_format($returnContiniousData->SIXMONTHS, 2) : 'NA' }}</td>
                                        <td data-label="1Y">{{ $returnContiniousData->ONEYEAR !='' ? number_format($returnContiniousData->ONEYEAR, 2) : 'NA' }}</td>
                                        <td data-label="2Y">{{ $returnContiniousData->TWOYEAR !='' ? number_format($returnContiniousData->TWOYEAR, 2) : 'NA' }}</td>
                                        <td data-label="3Y">{{ $returnContiniousData->THREEYEAR !='' ? number_format($returnContiniousData->THREEYEAR, 2) : 'NA' }}</td>
                                        <td data-label="5Y">{{ $returnContiniousData->FIVEYEAR !='' ? number_format($returnContiniousData->FIVEYEAR, 2) : 'NA' }}</td>
                                    </tr>
                                    @empty
                                    <tr>
                            ...
                            <table class="cmn_table ">
                                <thead>
                                    <tr style="border-width: 0;">
                                        ...
                                        <th style="border-right: 1px solid;">{{ $return_date_6_month.' - '.$return_date }} (6M)</th>
                                        <th style="border-right: 1px solid;">{{ $return_date_odd_1.' - '.$return_date_6_month }} (1Y)</th>
                                        <th>{{ $return_date_odd_2.' - '.$return_date_odd_1 }} (2Y)</th>
                                    </tr>
                                </thead>
                                <tbody class="retunr_discontinus_table">
                                @forelse($returnContiniousDis as $keyrc=>$returnContiniousDisData)
                                    <tr>
                                        <td data-label="Return">{{ $returnContiniousDisData->label}}</td>
                                        <td data-label="6M">{{ $returnContiniousDisData->SIXMONTHS !='' ? number_format($returnContiniousDisData->SIXMONTHS, 2) : 'NA' }}</td>
                                        <td data-label="1Y">{{ $returnContiniousDisData->ONEYEAR !='' ? number_format($returnContiniousDisData->ONEYEAR, 2) : 'NA' }}</td>
                                        <td data-label="2Y">{{ $returnContiniousDisData->TWOYEAR !='' ? number_format($returnContiniousDisData->TWOYEAR, 2) : 'NA' }}</td>
                                    </tr>
                                    @empty
                                    <tr>
                    ...
                    <div class="mt_15 fw-bold" style="border: 1px solid #fff; border-radius: 18px; padding: 10px;">
                        <p>{!! $fundWatch->performance_parameter_bottom !!}</p>
                    </div>
                ...
                <!-- <div class="return_less mt_15">
                    ...
                    <div class="mt_15 fw-bold" style="border: 1px solid #fff; border-radius: 18px; padding: 10px;">
                        <p class="text-white">{!! $fundWatch->index_rank_bottom !!}</p>
                    </div>
                </div> 
                ...
                <div class="return_less mt_15">
                    ...
                    <div class="mt_15 fw-bold" style="border: 1px solid #fff; border-radius: 18px; padding: 10px;">
                        <p class="text-white">{!! $fundWatch->rank_side !!}</p>
                    </div>
                </div>
                ...
                <div class="risk_adjust mt_15">
                    ...
                    <div class="mt_15 fw-bold" style="border: 1px solid #fff; border-radius: 18px; padding: 10px;">
                        <p class="text-dark">{!! $fundWatch->risk_adjust_bottom !!}</p>
                    </div>    
                </div>-->
                ...
                <div class="fund_compo mt_15">
                    ...
                    <div class="fund_compo_in">
                        <table class="cmn_table ">
                            ...
                            <tbody>
                            @if(!empty($fund_scrips))
                                @for ($i = 0; $i<10; $i++)
                                <tr>
                                    <td>{{ !empty($fund_scrips[$i]) ? $fund_scrips[$i]->scrip_name : 'NA' }}</td>
                                    <td>{{ !empty($fund_scrips[$i]) ? $fund_scrips[$i]->qty:'NA' }}</td>
                                    <td>{{ !empty($fund_scrips[$i]) ? $fund_scrips[$i]->qty1:'NA' }}</td>
                                    <td>{{ !empty($fund_scrips[$i]) ? $fund_scrips[$i]->qty2:'NA' }}</td>
                                    <td>{{ !empty($fund_scrips[$i]) ? $fund_scrips[$i]->qty3:'NA' }}</td>
                                    <td>{{ !empty($fund_scrips[$i]) ? $fund_scrips[$i]->qty4:'NA' }}</td>
                                </tr>
                                @endfor
                            @endif
                            </tbody>
                    ...
                    <div class="portfolio mt_15">
                        ...
                        <table class="cmn_table table table-bordered">
                            ...
                            <tbody class="">
                                @foreach($top_industries as $top_industrie)
                                    <tr style="border-width: 0;">
                                        <td>{{$top_industrie->industry}}</td>
                                        <td>{{ number_format($top_industrie->content_per,2)}}</td>
                                    </tr>  
                                @endforeach
                            </tbody>
                        </table>
                    ...
                    <div class="mt_15 fw-bold" style="border: 1px solid #fff; border-radius: 18px; padding: 10px;">
                        <p>{!! $fundWatch->composition_analysis_bottom !!}</p>
                    </div>
            ...
            <div class="content_right">
                ...
                <div class="m_fund">
                    <ul>
                        ...
                        <a href="{{ url('new-fundwatch') }}/{{$fund_code_encoded}}">
                ...
                <div class="m_fund arc_yr">
                    ...
                    @endforeach
                        ...
                        <input type="hidden" value="{{ $returnLessIndexRank }}" id="returnLessIndexRank">
                        <input type="hidden" value="{{ $getRankDecile }}" id="getRankDecile">
                        <input type="hidden" value="{{ $getRankQuartile }}" id="getRankQuartile">
...
@push('scripts')
                        ...
                        let loadingImage='<img  class="text-center mt-3" src="{{asset('themes/frontend/assets/v1/img/loading.gif')}}" alt="">';
                            ...
                            function drawVisualization() {
                                ...
                                decodedTitleElement.innerHTML = "{{ $fundMaster->fund_name }}";
