...
@section('content')
        ...
        <div class="page_detail">
            <div class="inner_padding">
                ...
                <div class="new_page">
                    ...
                    <div class="ja-filter">
                        <form action="{{ route('user.predictive.jensen-alpha') }}" method="GET">
                            <div class="ja-filter-grid">
                                <div class="ja-field">
                                    ...
                                    @error('fund_id')
                                        <div class="ja-error">{{ $message }}</div>
                                    @enderror
                                </div>

                                <div class="ja-info">
                                    <small>Date</small>
                                    <strong id="date">{{ $hasProjection ? \Carbon\Carbon::parse($graph_date[$actualCount - 1])->format('d M Y') : 'N/A' }}</strong>
                                </div>

                                <div class="ja-info">
                                    <small>Index</small>
                                    <strong id="indices_name">{{ $indices_details->name ?? 'N/A' }}</strong>
                                </div>

                                <div class="ja-info">
                                    <small>Index value</small>
                                    <strong id="indices_value">{{ $hasProjection ? number_format($closing_value[$actualCount - 1], 2) : '0.00' }}</strong>
                                </div>

                                <div class="ja-field">
                                    <label for="expected_index">Expected index</label>
                                    <input id="expected_index" type="number" name="expected_index"
                                        placeholder="Expected future index" value="{{ $expected_index ?? '' }}"
                                        step="0.01" min="0.01">
                                    @error('expected_index')
                                        <div class="ja-error">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="ja-actions">
                                    <button type="submit" name="duration" value="6"
                                        class="{{ (string) request('duration', '6') === '6' ? 'active' : '' }}">6m</button>
                                    <button type="submit" name="duration" value="1"
                                        class="{{ (string) request('duration') === '1' ? 'active' : '' }}">1y</button>
                                </div>
                            </div>
                        </form>
                    </div>
                    ...
                    @if ($hasProjection)
                        <div class="ja-summary">
                            <div class="ja-summary-item">
                                ...
                                <strong>{{ ($average_jensen_alpha ?? 0) >= 0 ? '+' : '' }}{{ number_format($average_jensen_alpha ?? 0, 2) }}%</strong>
                            </div>
                            <div class="ja-summary-item">
                                <span>Current NAV</span>
                                <strong>{{ number_format($currentNav, 2) }}</strong>
                            </div>
                            <div class="ja-summary-item">
                                <span>Projected NAV</span>
                                <strong>{{ number_format($projectedNav, 2) }}</strong>
                            </div>
                        </div>
                    @endif
                ...
                @if ($hasProjection && $disclaimer)
                    <div class="ja-note"><strong>Disclaimer:</strong> {{ $disclaimer }}</div>
                @endif
            </div>
        </div>
    </div>
    ...
    <script>
        function set_fund_select_val(fundId) {
            $.ajax({
                url: '{{ route('user.predictive.fund_details') }}?id=' + fundId,
                type: 'GET',
        ...
        var indexValues = @json($closing_value ?? []);
        var fundName = @json($fund_details->fund_name ?? 'Fund NAV');
        var indexName = @json($indices_details->name ?? 'Index');
