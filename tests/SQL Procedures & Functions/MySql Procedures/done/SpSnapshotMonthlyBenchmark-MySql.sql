DELIMITER $$
CREATE PROCEDURE `sp_snapshot_monthly_benchmark`(IN `p_entry_date` VARCHAR(10))
BEGIN
	...
	DROP TABLE IF EXISTS fund_temp;
		...
		SELECT final_data.name, final_data.monthly_change,final_data.ft_id FROM
		...
		WHERE mpx_fund_detail.entry_date = DATE_SUB(p_entry_date,INTERVAL DAYOFMONTH(p_entry_date)- 1 DAY))X
		INNER JOIN
		(SELECT closing_nav AS cur_value, fund_code FROM mpx_fund_detail
		WHERE mpx_fund_detail.entry_date = LAST_DAY(p_entry_date))Y
