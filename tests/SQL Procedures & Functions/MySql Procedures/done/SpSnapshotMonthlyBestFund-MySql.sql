...
CREATE PROCEDURE sp_snapshot_monthly_best_fund (
...
BEGIN
	...
	WHERE mpx_fund_detail.entry_date = TIMESTAMPADD(MONTH, -1, STR_TO_DATE(p_entry_date, 103)))X
	INNER JOIN
	...
	ON X.fund_code = Y.fund_code)Z INNER JOIN mpx_fund_master ON Z.fund_code = mpx_fund_master.fund_code
	INNER JOIN mpx_fund_type ON mpx_fund_master.fund_type_id = mpx_fund_type.ft_id
	) final_data ORDER BY final_data.weekly_change DESC
	LIMIT 10;
END;
//
