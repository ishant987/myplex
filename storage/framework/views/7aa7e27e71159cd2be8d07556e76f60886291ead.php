<?php $attributes = $attributes->exceptProps([
'label'=>'', 
'fieldFor'=>'', 
'fieldForClass'=>'', 
'for'=>'',
'class'=>'',
'error'=>'',
'required'=>'',
'info'=>''
]); ?>
<?php foreach (array_filter(([
'label'=>'', 
'fieldFor'=>'', 
'fieldForClass'=>'', 
'for'=>'',
'class'=>'',
'error'=>'',
'required'=>'',
'info'=>''
]), 'is_string', ARRAY_FILTER_USE_KEY) as $__key => $__value) {
    $$__key = $$__key ?? $__value;
} ?>
<?php $__defined_vars = get_defined_vars(); ?>
<?php foreach ($attributes as $__key => $__value) {
    if (array_key_exists($__key, $__defined_vars)) unset($$__key);
} ?>
<?php unset($__defined_vars); ?>
<div class="form-group has-primary row <?php echo e($class); ?> <?php echo e($error?'has-danger':''); ?>">
  <label for="<?php echo e($for?$for:''); ?>" class="col-sm-2 col-form-label">
  	<?php echo e($label?$label:''); ?> 
    <?php echo $fieldFor?'<span class="field_for '.$fieldForClass.'">'.$fieldFor.'</span>':''; ?> 
  	<?php if($required == 'true' || $required == 'y'): ?><span class="required">*</span><?php endif; ?>
  </label>
  <div class="col-sm-10">
    <?php echo e($slot); ?>  
    <?php if($info): ?>
    <div class="col-form-label info"><?php echo $info; ?></div>
    <?php endif; ?>  
    <?php if($error): ?>
    <div class="col-form-label"><?php echo e($error); ?></div>
    <?php endif; ?>
  </div>
</div><?php /**PATH /var/www/vhosts/new.myplexus.com/httpdocs/my-plexus/resources/views/components/form/group_lyt1_2_10.blade.php ENDPATH**/ ?>