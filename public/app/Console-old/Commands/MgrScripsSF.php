...
class MgrScripsSF extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'migration:scripssitefolks';
     ...
     * @var string
     */
    protected $description = 'Take data from tb_scrips table.';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $totRecords = $mgrDb->table('tb_scrips')->select('ScripName')->count();
            if ($totRecords > 0) {
                ...
                for ($i = 1; $i <= $noOfLot; $i++) {
                    ...
                    $dataRows = $mgrDb->table('tb_scrips')->offset($offset)->take($this->fetchRecords)->get();
                    // print_r($dataRows);die();
                    foreach ($dataRows as $record) {
                        // $this->info("scrip_name : " . $record->ScripName);
                        $store = new Scrips;
                        if (!$store->where(['scrip_name' => $record->ScripName, 'actual_scrip' => $record->ActualScrip])->exists()) {
                            /*Insert code here*/
                            $store->scrip_name = $record->ScripName;
                            $store->type = $record->Type;
                            $store->industry = $record->Industry;
                            $store->actual_scrip = $record->ActualScrip;
