...
class MgrMcapEpsValueMonthlyMPX extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'migration:mcapepsvaluemonthlymyplexus';
     ...
     * @var string
     */
    protected $description = 'Take data from TbMCAPEPS table.';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $totRecords = $mgrDb->table('TbMCAPEPS')->select('ScripName')->count();
            if ($totRecords > 0) {
                ...
                $lastCurrencyMigrateOn = SettingsModel::getSettingValue('mcap_eps_migrate');
                for ($i = 1; $i <= $noOfLot; $i++) {
                    $offset = ($i - 1) * $this->fetchRecords;
                    $dataRows = $mgrDb->table('TbMCAPEPS')->whereDate('EntryDate', '>=', $lastCurrencyMigrateOn)->offset($offset)->take($this->fetchRecords)->orderBy('EntryDate', 'ASC')->get();
                    ...
                    foreach ($dataRows as $record) {
                        ...
                        if ($lastCurrencyMigrateOn != $storeEntryDate) {
                            SettingsModel::where('option_key', 'mcap_eps_migrate')->update(['option_value' => $storeEntryDate, 'updated_id' => $defSprAdmnId]);
                        } else {
                            $lastCurrencyMigrateOn = SettingsModel::getSettingValue('mcap_eps_migrate');
                        }
                        $store = new McapEps;
