...
class MgrIndicesCompValueMonthlySF extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'migration:indicescompvaluemonthlysitefolks';
     ...
     * @var string
     */
    protected $description = 'Take data from tb_indicescomposition table.';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $totRecords = $mgrDb->table('tb_indicescomposition')->select('EntryDate')->count();
            if ($totRecords > 0) {
                ...
                for ($i = 1; $i <= $noOfLot; $i++) {
                    ...
                    $dataRows = $mgrDb->table('tb_indicescomposition')->offset($offset)->take($this->fetchRecords)->get();
