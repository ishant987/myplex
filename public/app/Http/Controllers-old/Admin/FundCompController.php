...
class FundCompController extends BaseController
{
    public $className;
    ...
    public function index(Request $request)
    {
        ...
        $fltrDataArr['industry'] = $request->has('fiy') ? Common::ampSafeOnUrl($request->query('fiy')) : '';
        $fltrDataArr['category'] = $request->has('fcy') ? $request->query('fcy') : '';
        ...
        $sortbyArr = ['fc_id' => __('admin.insertion_txt'), 'fund_code' => __('admin.fund.code_txt'), 'scrip_name' => __('admin.scrip.label_txt'), 'industry' => __('admin.industry_txt'), 'category' => __('admin.category_txt'), 'content_per' => __('admin.percentage_txt'), 'indices_per' => __('admin.fund.comp.idc_percentage_txt')];
        $orderbyArr = ['asc' => 'ASC', 'desc' => 'DESC'];

        $showEntryArr = ['value' => __('admin.sw_entry.options.value'), 'text' => __('admin.sw_entry.options.text')];

        $roleRights = ['delete' => App::hasAccessToMethod($this->className, 'admin.fund-comp.delete')];

        return view('themes.backend.pages.fund.comp-list', compact('dataListModel', 'data', 'listDataAtrArr', 'moduleAtrArr', 'sortbyArr', 'orderbyArr', 'fltrDataArr', 'perPage', 'sortBy', 'orderBy', 'showEntryArr', 'lastSavedDate', 'roleRights'));
    }
    ...
    public function deletedata()
    {
        ...
        try {
            DB::beginTransaction();
            $lastSavedDate = SettingsModel::getSettingValue('fund_comp');
            $delModel = FundComposition::where('entry_date', $lastSavedDate)->delete();
            if ($delModel > 0) {
                $fdMdl = FundComposition::orderBy('entry_date', 'desc')->first();
                if($fdMdl){
                ...
                $save = SettingsModel::where('option_key', 'fund_comp')->update(['option_value' => $entryDate, 'updated_id' => $loginAdminId]);
                if ($save == 0) {
                    DB::rollBack();
