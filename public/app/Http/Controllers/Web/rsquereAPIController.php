...
class rsquereAPIController extends BaseController
{
    public $className;
    public $class_id;
    ...
    public function r_squere_calculator_mar(Request $request)
    {
        $input = $request->all();
        if(isset($input['search']) && $input['search'] == 'Search')
        {
    ...
    public function correlation($arr1, $arr2) {
        ...
        // Check if both arrays have the same number of elements
        if ($n != count($arr2) || $n == 0) {
            return null; // Correlation is undefined if arrays have different lengths or are empty
        }
        ...
        $sum2 = array_sum($arr2);
        // dd($sum2);
        $sum1Sq = array_sum(array_map(function ($x) { return pow($x, 2); }, $arr1));
        $sum2Sq = array_sum(array_map(function ($x) { return pow($x, 2); }, $arr2));
