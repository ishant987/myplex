...
class RatioAnalysisController extends Controller
{
    ...
    public function return_ratio(Request $request){
      ...
      $data['indices'] = DB::table('indices_master')->whereNotIn('name', ['Gold', 'S&P 500 Index', 'MSCI Emerging Markets Index'])->where('status',1)->get();
        ...
        if($request->has('Category') && $request->Category=='by_category'){
            ...
            if($request->ranking == 'range'){
                ...
                [
                    ...
                    'ranking'.'month' => 'Please select first period month',
                    'ranking'.'year' => 'Please select first period year',
                    'ranking'.'month_second' => 'Please select second period month',
                    'ranking'.'year_second' => 'Please select second period year',
                ...
                $start_date = $this->getFirstDate($month, $year);
                $end_date = $this->getlastdate($month_second, $year_second);
...
function getFirstDate($month, $year)
{
    ...
    return  date('Y-m-d', strtotime($firstDayOfMonth));
}
