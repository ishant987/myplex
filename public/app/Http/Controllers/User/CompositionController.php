    ...
    public function boomers(Request $request)
    {
        ...
        if ($request->has('Category') && $request->Category == 'by_fund') {
            ...
            foreach ($fund_wise_scrip as $sc) {
                ...
                // $scrip_results = DB::table(DB::raw('(SELECT scrip_name, fund_code, SUM(content_per) as c_start 
                // FROM mpx_fund_composition_corpus_entry 
                // WHERE corpus_entry_date = "'.$startDate.'" 
                //   AND fund_code IN ("'.implode('","', $fund_code_id_arra).'") 
                //   AND composition_entry_date = "'.$startDate.'" 
                //   AND scrip_name = "'.$sc->scrip_name.'" 
                // GROUP BY fund_code) as start'))
                // ->join(DB::raw('(SELECT scrip_name, fund_code, SUM(content_per) as c_end 
                //     FROM mpx_fund_composition_corpus_entry 
                //     WHERE corpus_entry_date = "'.$endDate.'" 
                //     AND fund_code IN ("'.implode('","', $fund_code_id_arra).'") 
                //     AND composition_entry_date = "'.$endDate.'" 
                //     AND scrip_name = "'.$sc->scrip_name.'" 
                //     GROUP BY fund_code) as end'), 'start.fund_code', '=', 'end.fund_code')
                // ->select('start.scrip_name', 
                // DB::raw('SUM(((end.c_end - start.c_start) / start.c_start) * 100) as percentage_change'))
                // ->groupBy('start.scrip_name')
                // ->get();

                                // Convert the array of fund codes into a properly formatted string for SQL
                $fund_codes = "'" . implode("','", $fund_code_id_arra) . "'";  // Add quotes around each value

                $scrip_results = DB::select(
                    'SELECT start.scrip_name, 
                            SUM(((end.c_end - start.c_start) / start.c_start) * 100) AS percentage_change
                    FROM (
                        SELECT scrip_name, fund_code, SUM(content_per) as c_start  
                        FROM mpx_fund_composition_corpus_entry
                        WHERE corpus_entry_date = ? 
                        AND fund_code IN ('.$fund_codes.') 
                        AND composition_entry_date = ? 
                        AND scrip_name = ?
                        GROUP BY fund_code
                    ) as start
                    JOIN (
                        SELECT scrip_name, fund_code, SUM(content_per) as c_end  
                        FROM mpx_fund_composition_corpus_entry 
                        WHERE corpus_entry_date = ? 
                        AND fund_code IN ('.$fund_codes.') 
                        AND composition_entry_date = ? 
                    ...
                    ON start.fund_code = end.fund_code
                    GROUP BY start.scrip_name',
                    [
                        $startDate,    // First ?
                        $startDate,    // Second ?
                        $sc->scrip_name, // Third ?
                ...
                if (!empty($scrip_results)) {
                    foreach ($scrip_results as $result) {
                        if( $result->scrip_name !='' && isset($result->scrip_name)){
                            $scrip_push = [
                                'scrip_name' => $result->scrip_name,  // Access the property from the result object
                                'percentage_change' => $result->percentage_change  // Access the calculated percentage change
                            ];
                            array_push($result_scrip_array, $scrip_push);
                        }
            ...
            foreach($fund_wise_industry as $sc) {
                ...
                $scrip_results = DB::select(
                    'SELECT start.industry, 
                            SUM(((end.c_end - start.c_start) / start.c_start) * 100) AS percentage_change
                    FROM (
                        SELECT industry, fund_code, SUM(content_per) as c_start  
                        FROM mpx_fund_composition_corpus_entry
                        WHERE corpus_entry_date = ? 
                        AND fund_code IN ('.$fund_codes.') 
                        AND composition_entry_date = ? 
                    ...
                    JOIN (
                        SELECT industry, fund_code, SUM(content_per) as c_end  
                        FROM mpx_fund_composition_corpus_entry 
                        WHERE corpus_entry_date = ? 
                        AND fund_code IN ('.$fund_codes.') 
                        AND composition_entry_date = ? 
                ...
                if (!empty($scrip_results)) {
                    foreach ($scrip_results as $result) {
                        if( $result->industry !=''){
                            $scrip_push = [
                                'industry_name' => $result->industry,  // Access the property from the result object
                                'percentage_change' => $result->percentage_change  // Access the calculated percentage change
                            ];
                            array_push($result_industry_array, $scrip_push);
                        }
                    }
                }
            ...
            foreach ($fund_wise_scrip as $sc) {
                ...
                if (isset($fund_wise_corpus_amount_entry_date) && isset($fund_wise_corpus_amount_last_date) && ($fund_wise_corpus_amount_entry_date->c_entry != 0) && ($fund_wise_corpus_amount_last_date->c_entry != 0)) {
                    ...
                    $calculation_array = array(
                        ...
                        'scrip_name' => $sc->scrip_name,
                        'sum_corpus_start_date' => $sum_amount_start_date->c_entry,
                        'sum_corpus_end_date' => $sum_amount_end_date->c_entry,
                        'fund_wise_corpus_start_date' => $fund_wise_corpus_amount_entry_date->c_entry,
                        'fund_wise_corpus_end_date' => $fund_wise_corpus_amount_last_date->c_entry,
                    ...
                    array_push($calculation_data_array, $calculation_array);

                }
            }
            // dd($fund_code_id_arra,$calculation_data_array,$result_scrip_array);
